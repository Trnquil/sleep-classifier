...
class GeminiService(object):
    ...
    model_path = utils.get_project_root().joinpath("GEMINI/results/mesa/lenet5/22-12-16_15-11-35_045368/model.pt")
    
    def load_model():
        
        config=create_config(GeminiService.args)
        model=build_model(config)
        model.load_state_dict(torch.load(GeminiService.model_path)['model_state_dict'])
        return model
