...
class MotionService(object):
    ...
    def load(motion_file, delimiter=' '):
        motion_array = pd.read_csv(str(motion_file))
        ...
        motion_array = motion_array.values/64
        
        #data value frequency in Hertz
        data_frequency = motion_array[0][0]

        motion_array = motion_array[1:][:]
        
        #timestamps created at the intervals specified by data_frequency
        timestamps = [(i*(1/data_frequency)) for i in range(len(motion_array))]
...
#subject_id='S01'
#MotionService.load_raw(subject_id)